      ...
      {showPopup && (
        <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-6 rounded-xl shadow-lg w-80 text-center">
            <h2 className="text-lg font-semibold text-gray-700 mb-4">Select Loan Option</h2>
            {["Long Term Loan", "Short Term Loan", "Mid Term Loan"].map((cat) => (
              <label key={cat} className="flex items-center gap-2 text-blue-500 cursor-pointer mb-2">
                <input
                  type="radio"
                  value={cat}
