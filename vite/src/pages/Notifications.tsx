...
const Notifications: React.FC<PropTypes> = ({ toNotifications }) => {
  const { message, readMessage, read, deposits } = useContext(AppContext);
        ...
        message.map((msg, index) => (
          <div key={index} className="p-3 bg-white shadow-md rounded-lg my-2">
            ...
            <span className="text-gray-600 text-sm">
              Your user ID is: <strong>{msg.userId}</strong>
            </span>
      ...
      {deposits && deposits.length > 0 ? (
        deposits.map((deposit, index) => (
          <div key={index} className="p-3 bg-white shadow-md rounded-lg my-2">
            ...
            <p className="text-xs text-blue-500 font-bold">Congratulations!  Your  <span className="text-black font-bold">{deposit.amount}</span>  deposit has been successfully added to your account. Your funds are now available,
               and you can continue managing your finances with ease. Thank you for banking with us</p>
